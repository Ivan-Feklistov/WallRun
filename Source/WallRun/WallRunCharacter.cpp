...
AWallRunCharacter::AWallRunCharacter()
{
	...
	WallRunComp = CreateDefaultSubobject<UWallRunComponent>("WallRunComp");
...
void AWallRunCharacter::Jump()
{
	...
	if (WallRunComp->bCanJumpFromWall)
	{
		if (JumpSound)
			UGameplayStatics::PlaySound2D(this, JumpSound);
		WallRunComp->WallJump();
	}
	else
		if (JumpSound && CanJump())
			UGameplayStatics::PlaySound2D(this, JumpSound);
	...
	else
	{
		if (Value != 0.0f)
		{
			...
			{	
				...
				if (WallRunComp->IsCharacterMovingBackwards())
				{
					Value = FMath::Clamp(Value, 0.0f, 99999.0f);
				}
			}
			// add movement in that direction
			AddMovementInput(GetActorForwardVector(), Value);
